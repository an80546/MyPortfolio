...
- Add controlled expand/collapse behavior only where long content makes mobile scanning harder.
...
Skill cards stay concise on mobile. The main skills grid shows the logo and title without an extra description interaction. Certification cards keep their existing description and tool tags, stacked in a single column on narrow screens.
...
No data model changes are required. Expand/collapse state can live inside `ProfileCard` because the state is purely presentational. Existing props and events should remain unchanged.

## Testing

Run `npm run build` after implementation. Verify mobile layouts at narrow widths around 360px and 430px, plus a desktop width to confirm existing layouts were not broken. Check that expanding the home intro works with a keyboard-accessible button.

## Open Assumptions

The mobile version should keep all current content available. The home intro may be collapsed by default on mobile, but expanded content should still reveal the original text.
